...
namespace DS4Windows
{
    public class DS4Devices
    {
        ...
        public static DS4Device getDS4Controller(int index)
        {
            lock (Devices)
            {
                DS4Device device = null;
                if(Devices.Count > index)
                    device = Devices.Values.ElementAt(index);

                return device;
            }
        }

        public static DS4Device getNextDS4Controller()
        {
            lock (Devices)
            {
                DS4Device device = null;
                if (Devices.Count > controllerIndex)
                    device = Devices.Values.ElementAt(controllerIndex++);

                return device;
            }
        }

        public static void ResetControllerIndex()
        {
            controllerIndex = 0;
        }
